...
function QuestionnaireProgress({
  ...
  return (
    <QuestionnairePrimitive.Progress
      ...
      /* O primitivo monta "Question 1 of 3" em inglês. Aqui não é preferência de idioma: é a
         tela do paciente, e ele lê isso às 6 da manhã com dor. */
      render={
        render ??
        ((renderProps, state) => (
          <div {...renderProps}>{children ?? `Pergunta ${state.current} de ${state.total}`}</div>
        ))
      }
      {...props}
    />
